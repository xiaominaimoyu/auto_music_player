...
class EventPlayer(QObject):
    ...
    def play(self, events, interrupt_mode="pause", start_index=0, score_name=""):
        ...
        self._thread = threading.Thread(
            target=self._run,
            args=(list(events), interrupt_mode, int(start_index), str(score_name)),
            daemon=True,
        )
        self._thread.start()
    ...
    def _run(self, events, interrupt_mode, start_index, score_name):
        total = len(events)
        ...
        base_ms = events[start_index].t_ms if 0 <= start_index < total else 0.0
